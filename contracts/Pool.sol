...
contract Pool{
    ...
    function repay(address _token, address _for, uint _amount) public returns (uint actualAmount){
        (,,DToken dToken,,,) = Router(router).assets(_token);
        uint amount = _amount * dToken.totalSupply() / Router(router).totalDebts(_token);
    ...
    function liquidate(LiquidateArgs memory _param) external returns (uint actualAmount, uint burnedAmount){
        (,,DToken dToken,,,) = Router(router).assets(_param.debtAsset);
        uint amount = _param.amount * dToken.totalSupply() / Router(router).totalDebts(_param.debtAsset);
