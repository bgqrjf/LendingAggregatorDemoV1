...
describe("DepositLogic Tests", function () {
  ...
  beforeEach(async () => {
    ...
    // _aaveTokenAddress
    providerAAVE = await ProviderAAVE.deploy(
      aPool.address,
      wETH.address,
      wETH.address
    );
  ...
  describe("AAVE tests", function () {
    describe("supply tests", function () {
      it("should supply ERC20 properly", async () => {
        ...
        await token0
          .connect(supplier0)
          .approve(pool.address, token0SupplyAmount);
        let tx = await pool.supply(
          token0.address,
          supplier0.address,
          token0SupplyAmount,
          true
      ...
      it("should supply ETH properly", async () => {
        ...
        let tx = await pool.supplyETH(supplier0.address, true, {
      ...
      it("should supply twice properly", async () => {
        ...
        await token0
          .connect(supplier0)
          .approve(pool.address, token0SupplyAmount * 2);
        await pool.supply(
          token0.address,
          supplier0.address,
          token0SupplyAmount,
        ...
        let MockLibraryTest = await ethers.getContractFactory(
          "MockLibraryTest"
        );
        let mockLibraryTest = await MockLibraryTest.deploy();
        let config = await ethers.getContractAt(
          "Config",
          await router.config()
        );

        let bitMap0 = await config.userDebtAndCollateral(supplier0.address);
        let collateralable0 = await mockLibraryTest.isUsingAsCollateral(
          bitMap0,
          asset.index
        );
        ...
        let collateralable1 = await mockLibraryTest.isUsingAsCollateral(
          bitMap1,
          asset.index
        );
    ...
    describe("withdraw tests", function () {
      it("should withdraw ERC20 properly", async () => {
        ...
        let asset = await router.assets(token0.address);
        let sToken = await ethers.getContractAt("SToken", asset.sToken);
        let withdrawAmount = (await sToken.balanceOf(supplier0.address)).div(
          10
        );
        let tx = await router.withdraw(
          token0.address,
          supplier1.address,
          withdrawAmount,
          false
      ...
      it("should withdraw ETH properly", async () => {
        ...
        expect(supplier1balance1.sub(supplier1balance0)).to.equal(
          "100000000000000000"
        );
    ...
    describe("borrow tests", function () {
      it("should borrow ERC20 properly", async () => {
        let token0SupplyAmount = new ethers.BigNumber.from(
        ...
        await usdt
          .connect(borrower0)
          .approve(pool.address, usdtColletralAmount);
        await pool
          .connect(borrower0)
          .supply(usdt.address, borrower0.address, usdtColletralAmount, true);
        ...
        let tx = await router
          .connect(borrower0)
          .borrow(token0.address, borrower0.address, borrowAmount);
        let receipt = await tx.wait();
        m.log("gas used:", receipt.gasUsed);
      ...
      it("should borrow ETH properly", async () => {
        await pool
          .connect(supplier0)
          .supplyETH(supplier0.address, true, {
            value: ethers.BigNumber.from("1000000000000000000000"),
          });
        ...
        let tx = await router
          .connect(borrower0)
          .borrow(ETHAddress, borrower1.address, borrowAmount);
        let receipt = await tx.wait();
        m.log("gas used:", receipt.gasUsed);
      ...
      it("should borrow ERC20 twice properly", async () => {
        ...
        await router
          .connect(borrower0)
          .borrow(token0.address, borrower0.address, borrowAmount.div(2));
        let tx = await router
          .connect(borrower0)
          .borrow(token0.address, borrower0.address, borrowAmount.div(2));
        let receipt = await tx.wait();
    ...
    describe("repay tests", function () {
      it("should repay ERC20 properly", async () => {
        ...
        await token0
          .connect(borrower0)
          .approve(pool.address, borrowAmount.add(5006809261));
        let tx = await pool
          .connect(borrower0)
          .repay(token0.address, borrower0.address, borrowAmount);
        let receipt = await tx.wait();
        m.log("gas used:", receipt.gasUsed);
      ...
      it("should repay ETH properly", async () => {
        ...
        await router
          .connect(borrower0)
          .borrow(ETHAddress, borrower0.address, borrowAmount);

        let tx = await pool
          .connect(borrower0)
          .repayETH(borrower0.address, borrowAmount, {
            value: ethers.BigNumber.from("11000000000000000000"),
          });
        let receipt = await tx.wait();
        m.log("gas used:", receipt.gasUsed);
      ...
      it("should repay twice ERC20 properly", async () => {
        ...
        await token0
          .connect(borrower0)
          .approve(pool.address, borrowAmount.add(5424043365));
        await pool
          .connect(borrower0)
          .repay(token0.address, borrower0.address, borrowAmount.div(2));
        let tx = await pool
          .connect(borrower0)
          .repay(token0.address, borrower0.address, borrowAmount.div(2));
        let receipt = await tx.wait();
