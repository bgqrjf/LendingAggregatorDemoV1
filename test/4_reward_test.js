...
describe("reward tests", function () {
  ...
  it("should claim comp properly", async () => {
    await token0
      .connect(supplier0)
      .approve(pool.address, "1000000000000000000000000");

    m.log("index:", await comptroller.compSupplyState(cToken0.address));
    m.log(
      "rewardData",
      await router.rewardData(
        supplier0.address,
        providerCompound.address,
        token0.address
      )
    ...
    await router
      .connect(borrower0)
      .borrow(token0.address, borrower0.address, "100000000000000000000000");
