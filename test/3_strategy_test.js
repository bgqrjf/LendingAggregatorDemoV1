...
describe("Strategy Tests", function () {
  ...
  beforeEach(async () => {
    ...
    let compContracts = await compound.deployContracts({
      token0: token0,
      usdt: usdt,
    });
  ...
  describe("strategy tests", function () {
    it("should supply with strategy properly", async () => {
      let aaveSupplyRate = await providerAAVE.getCurrentSupplyRate(
        token0.address
      );
      m.log("aaveSupplyRate:", aaveSupplyRate);
      let compoundSupplyRate = await providerCompound.getCurrentSupplyRate(
        token0.address
      );
      ...
      let strategyForSupply = await strategy.getSupplyStrategy(
        [providerAAVE.address, providerCompound.address],
        token0.address,
        supplyAmount.div(100).mul(95),
        router.address
      );
      m.log(
        `preview strategy for supply ${supplyAmount} token0 via Lending Aggregator`
      );
      ...
      m.log("aaveSupplyRate:", aaveSupplyRate);
      compoundSupplyRate = await providerCompound.getCurrentSupplyRate(
        token0.address
      );
      m.log("compoundSupplyRate:", compoundSupplyRate);
    });
    ...
    it("should withdraw with strategy properly", async () => {
      ...
      m.log(
        `borrow ${token0SupplyAmount.div(
          10
        )} from cToken0 again to alter compound interest rate`
      );
      ...
      let log = router.interface.parseLog(
    ...
    it("should borrow with strategy properly", async () => {
      ...
      let aaveBorrowRate = await providerAAVE.getCurrentBorrowRate(
        token0.address
      );
      m.log("aaveBorrowRate:", aaveBorrowRate);
      let compoundBorrowRate = await providerCompound.getCurrentBorrowRate(
        token0.address
      );
      ...
      m.log("aaveBorrowRate:", aaveBorrowRate);
      compoundBorrowRate = await providerCompound.getCurrentBorrowRate(
        token0.address
      );
      m.log("compoundBorrowRate:", compoundBorrowRate);
    });
    ...
    it("should repay with strategy properly", async () => {
      ...
      let tx = await pool
        .connect(borrower0)
        .repay(token0.address, borrower0.address, repayAmount);
