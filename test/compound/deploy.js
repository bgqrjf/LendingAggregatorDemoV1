...
exports.deployContracts = async ({ token0, usdt }) => {
  ...
  await comp.transfer(
    comptroller.address,
    await comp.balanceOf(deployer.address)
  );
  ...
  await comptroller._setCollateralFactor(
    cToken0.address,
    ethers.BigNumber.from("790000000000000000")
  );
  await comptroller._setCollateralFactor(
    cUSDT.address,
    ethers.BigNumber.from("840000000000000000")
  );
  await comptroller._setCollateralFactor(
    cETH.address,
    ethers.BigNumber.from("825000000000000000")
  );
